...
# ボタン
st.subheader("AIエンジニアリング実践講座を頑張る？")
if st.button("ここクリック！"):
    st.success(f"{name}も頑張ろう！")

# チェックボックス
st.subheader("やる気チェック")
if st.checkbox("私はやる気があります"):
    st.info(f"ナイスモチベーション！{name}さん！")

# スライダー
st.subheader("モチベーション度")
age = st.slider("モチベーション", 0, 100, 10)
st.write(f"{name}さんのモチベーション: {age}")
if age>=50:
  st.info(f"ナイスモチベーション！{name}さん！")
else:
  st.info(f"もっとやる気持っていこう！{name}さん！")
